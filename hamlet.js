...
;
  ...
  this.Hamlet.toHtml = function(html) {
    var attrs, class_attr, classes, content, delete_comment, id, id_attr, ids, innerHTML, last_tag_indent, line, needs_space, oldp, oldt, pos, push_innerHTML, si, tag_attrs, tag_name, tag_portion, tag_stack, ti, unindented, _i, _len, _ref, _ref10, _ref11, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
    content = [];
    tag_stack = [];
    last_tag_indent = 0;
    needs_space = false;
      ...
      if (unindented.length === 0) {
        ...
        if (pos <= last_tag_indent) {
          ...
          if (tag_stack.length > 0 && pos === last_tag_indent) {
            ...
            last_tag_indent = ((_ref7 = tag_stack[tag_stack.length - 1]) != null ? _ref7[0] : void 0) || 0;
            content.push("</" + oldt + ">");
          }
        }
        if (unindented[0] === '>') {
        ...
        if (unindented[0] !== '<') {
          ...
          _ref8 = tag_name.split('#'), tag_name = _ref8[0], ids = 2 <= _ref8.length ? __slice.call(_ref8, 1) : [];
          if (ids.length !== 0) {
            if (ids.length > 1) throw "found multiple ids: " + ids.join(',');
            _ref9 = ids[0].split('.'), id = _ref9[0], classes = 2 <= _ref9.length ? __slice.call(_ref9, 1) : [];
            if (classes.length !== 0) {
              tag_name = tag_name + '.' + classes.join('.');
            }
            id_attr = id;
          }
          _ref10 = tag_name.split('.'), tag_name = _ref10[0], classes = 2 <= _ref10.length ? __slice.call(_ref10, 1) : [];
          if (classes.length !== 0) class_attr = classes;
          ...
          if (emptyTags[tag_name]) {
            ...
            if (tag_attrs.length === 0 && !id_attr && (class_attr || []).length === 0) {
              ...
              if (id_attr) attrs.unshift(["id", id_attr]);
              content.push("<" + tag_name + " " + (join_attrs(attrs)) + ">");
            }
            if (innerHTML.length !== 0) push_innerHTML(innerHTML);
          }
        }
      }
    }
    while (tag_stack.length > 0) {
      _ref11 = tag_stack.pop(), oldp = _ref11[0], oldt = _ref11[1];
      content.push("</" + oldt + ">");
    }
    return content.join("");
  };
